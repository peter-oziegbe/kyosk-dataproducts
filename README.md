...
The repository is primarily owned by the [data team](https://github.com/orgs/Kyosk-Digital/teams/data-team). They have central control of things like managing shared code, CI/CD actions etc. The rest of the teams are controlled via the codeowners file located in `$Root/.*github/CODEOWNERS`*  
...
- Operations
    ...
    To add another squad's data product as a dependency in your repo (e.g. the `audit.proto` file in platform's repo), do this:
    1. In your squad's "buf.yaml" file, add the "platform" artifact as a dependency under the "deps" section.
        e.g. 
        ```
        deps:
    ...
    2. Open a terminal and run the following commands:
        ...
        a) First log in to Buf Schema Registry:

            buf registry login
                > username: my-user-name
                > token: my-token

        b) Navigate to the directory which has your squad's "buf.yaml" file. e.g. for retail experience: 

            cd retail_exp/protos

        c) Run "buf mod update". This will update your squad's dependencies
